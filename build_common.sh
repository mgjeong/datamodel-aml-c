...
install_dependencies() {
    ...
    ./build_common.sh --with_dependencies=${AML_WITH_DEP} --target_arch=${AML_TARGET_ARCH} --build_mode=${AML_BUILD_MODE} --logging=${AML_LOGGING} --exclude_protobuf=${AML_EXCLD_PROTOBUF}
    if [ $? -ne 0 ]; then 
